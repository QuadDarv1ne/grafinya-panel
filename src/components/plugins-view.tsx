...
export function PluginsView() {
  const { plugins, setPlugins, connectionStatus, isLoading, setIsLoading } =
    useGraphinyaStore();
    ...
    <div className="space-y-6">
      ...
      {!isConnected && (
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 flex items-center gap-3">
          <Plug className="h-5 w-5 text-amber-500 shrink-0" />
      ...
      {!isLoading && plugins.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {plugins.map((plugin) => {
            ...
            return (
              <Card key={plugin._id} className="hover:shadow-md transition-all">
